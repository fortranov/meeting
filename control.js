...
let _dragControlId   = null;
let _dragParentId    = null;
let collapsedTasks   = new Set();
let controlDates     = new Set();

const _dp = { taskId: null, controlId: null, start: null, picking: null, hoverDate: null, viewYear: null, viewMonth: null };
...
function bindEvents() {
  ...
  document.getElementById('addMeetingBtn').onclick    = () => openControlModal();
  document.getElementById('saveMeeting').onclick      = saveControl;
  document.getElementById('saveTask').onclick         = saveTask;
  document.getElementById('deleteMeetingBtn').onclick = deleteControl;
...
async function loadTimeline() {
  ...
  const res   = await fetch(`api.php?action=control_timeline&start=${start}&days=${VISIBLE_DAYS}`);
  const data  = await res.json();
  timelineControls = data.controls || [];
  renderTimeline();
}
...
function renderTimeline() {
  ...
  const rows = [];
  controlDates = new Set();
  timelineControls.forEach(c => {
    controlDates.add(c.control_date);
    rows.push({ type: 'meeting', id: c.id, meetingId: c.id, title: c.title, start: c.control_date, end: c.control_date, status: '', level: 0 });
    (c.tasks || []).forEach(t => pushTaskRows(rows, t, c.id, 0));
  });
  ...
  const header = renderMonthRow(days, tpl) + `
    <div class="timeline-row header" style="grid-template-columns:${tpl}">
      ...
      <div class="timeline-cell left-col left-1">Контроль за месяц / задача</div>
  ...
  timelineTable.querySelectorAll('[data-action="edit-control"]').forEach(btn =>
    btn.onclick = () => openControlModal(Number(btn.dataset.id))
  );
  ...
  if (canDrag) {
    timelineTable.querySelectorAll('.timeline-row.task').forEach(row => {
      ...
      cell.addEventListener('click', () => {
        ...
        const task = findTask(taskId, timelineControls);
        if (!task) return;
...
function pushTaskRows(rows, task, controlId, level, parentId = null, topLevelTaskId = null) {
  ...
  (task.children || []).forEach(ch => pushTaskRows(rows, ch, controlId, level + 1, task.id, myTopLevelId));
}
...
function renderActions(row) {
  ...
  const editBtn = row.type === 'meeting'
    ? `<button class="btn-icon" title="Редактировать контроль" data-action="edit-control" data-id="${row.meetingId}">${editSvg}</button>`
...
function renderDayHeader(day) {
  ...
  const meetingDay = controlDates.has(isoDay) ? 'meeting-day' : '';
...
function renderRangeCell(day, start, end, status = '', directionColor = null, responsible = '') {
  const weekend    = day.getDay() === 0 || day.getDay() === 6 || isHoliday(day) ? 'weekend' : '';
  const d          = toISO(day);
  const meetingDay = controlDates.has(d) ? 'meeting-day' : '';
...
function openControlModal(id = null) {
  ...
  document.getElementById('meetingModalTitle').textContent = id ? 'Редактировать контроль за месяц' : 'Создать контроль за месяц';
  ...
  const c = timelineControls.find(x => Number(x.id) === id);
  if (!c) return;
  meetingId.value   = c.id;
  meetingName.value = c.title;
  meetingDate.value = c.control_date;
...
function openTaskModal({ taskId = null, meetingId: mid, parentTaskId = '' }) {
  ...
  if (taskId) {
    ...
    const parent = findTask(Number(parentTaskId), timelineControls);
    if (parent) {
...
async function saveControl() {
  ...
  const payload = { id: meetingId.value, title: meetingName.value, control_date: meetingDate.value, use_template: useTemplate ? 1 : 0 };
  const res  = await fetch('api.php?action=control_save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
  const data = await res.json();
...
async function saveTask() {
  const payload = {
    ...
    control_id:     Number(taskMeetingId.value),
  ...
  const res  = await fetch('api.php?action=control_task_save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
  const data = await res.json();
  if (data.error) return alert(data.error);
...
async function deleteControl() {
  ...
  if (!id) return;
  if (!confirm('Удалить контроль за месяц и все связанные с ним задачи?')) return;
  const res  = await fetch('api.php?action=control_delete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
  const data = await res.json();
  if (data.error) return alert(data.error);
  closeModal('meetingModal');
  await loadTimeline();
...
async function deleteTask() {
  ...
  const res  = await fetch('api.php?action=control_task_delete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
  const data = await res.json();
  if (data.error) return alert(data.error);
  closeModal('taskModal');
  await loadTimeline();
...
function setupDragDrop() {
  timelineTable.querySelectorAll('.drag-handle').forEach(handle => {
    handle.ondragstart = e => {
      ...
      _dragControlId = Number(row.dataset.meetingId);
  ...
  timelineTable.querySelectorAll('[data-task-id]').forEach(row => {
    row.ondragover = e => {
      ...
      const targetControl = Number(row.dataset.meetingId);
      if (targetControl !== _dragControlId) return;
    ...
    row.ondrop = async e => {
      ...
      const control = timelineControls.find(c => Number(c.id) === _dragControlId);
      if (!control) return;
      const siblings = _dragParentId !== null
        ? (findTask(_dragParentId, [control])?.children || [])
        : (control.tasks || []);
      ...
      renderTimeline();
      await fetch('api.php?action=control_task_reorder', {
        method: 'POST',
...
async function saveDateRange(taskId, startDate, endDate) {
  const res  = await fetch('api.php?action=control_task_dates', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
function findTask(id, controls) {
  ...
  for (const c of controls) { const f = walk(c.tasks || []); if (f) return f; }
  return null;
}

const SUPPRESSED_KEY = 'controlConflictSuppressed';
